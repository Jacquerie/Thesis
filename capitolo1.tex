...
\begin{example}
	Sia \(L\) il grafo su tre vertici con due archi, rappresentato a sinistra. Allora \(L^2\) è rappresentato a destra.
    \begin{figure}
        \tikzstyle{vertex}=[circle,draw=black!100,thick,minimum size=1cm]
        \begin{tikzpicture}[scale=2]
            \node (b) at (1,0) [vertex] {b};
            \node (a) at (3,0) [vertex] {a};
            \node (c) at (2,1.732) [vertex] {c};
            \draw (b.east) -- (a.west) [thick];
        \end{tikzpicture}
    \end{figure}
\end{example}
