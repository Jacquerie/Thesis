...
Una \emph{codifica propria dell'informazione} in esse contenute è una mappa dalle stringhe a un insieme finito di simboli tale che a stringhe distinguibili vengano assegnati simboli distinti.
...
Sia \(G\) un grafo. Chiamiamo \emph{nucleo} un insieme indipendente e massimale per l'inclusione. Avremo bisogno di due lemmi di teoria dei grafi e di una seconda nozione di potenza di grafo. 
\begin{lemma}
...
Enunciamo i seguenti due lemmi omettendone la dimostrazione.
\begin{lemma}
	\label{boundslemma} Sia \(G\) unione di grafi completi a due a due disgiunti e \(\sim\) la relazione d'equivalenza definita nela precedente lemma. Denotiamo con \([v]\) la classe d'equivalenza di \(v\) per tale relazione e definiamo
...
\end{theorem}
\begin{proof}
	Per il lemma \ref{ptyplemma} \((1)\), per ogni \(\lambda>0\) esiste \(K\) tale che, per \(t\) sufficientemente grande, \(\mathbf{P}^t(T^t(P))\ge 1-\lambda\). Per un tale \(t\) sia \(U_{t}\) un sottoinsieme di \(V^t\) tale che \(\mathbf{P}^t(U_{t})\ge 1-\varepsilon\). Ne deriva allora che 
	\begin{equation}
	...
	Ricordando inoltre il lemma \ref{ptyplemma} \((2)\)abbiamo 
	\begin{equation}
...
\begin{proof}
	...
	\end{equation}
	ed osservare che il secondo termine è maggiorato da \(\mathbf{P}^t\Big(\overline{T^{t}(P)}\Big)\), che per il lemma \ref{ptyplemma} \((1)\) possiamo supporre essere più piccolo di \(\varepsilon/2\). Per stimare il primo termine osserviamo innanzitutto che
	...
	\end{equation}
	Inoltre il lemma \ref{ptyplemma} \((2)\) implica che se \(\mathbf{A}\) è una sequenza di nuclei \(R-\)tipica allora 
	\begin{equation}
