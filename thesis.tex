...
L'entropia di grafo fu introdotta da Janos K\"orner come soluzione del seguente problema: assegnare ad un grafo una quantità che ne rappresenti la complessità ma che ammetta una interpretazione in termini di teoria dell'informazione.

Ciò fu ottenuto interpretando un grafo come grafo di distinguibilità dei simboli emessi da una sorgente discreta. Sia \(X\) una sorgente che ad ogni istante discreto emetta un simbolo \(v_i\) nell'alfabeto \(\{v_1\dots v_n\}\) con corrispondente probabilità \(p_i\). Supponiamo però di non essere necessariamente in grado di distinguire due simboli distinti, ed anzi rappresentiamo questa relazione di distinguibilità con un grafo: esso avrà come vertici i simboli \(\{v_1\dots v_n\}\) ed un arco fra \(v_i\) e \(v_j\) soltanto se tali simboli sono distinguibili. Tale relazione è simmetrica, ma non necessariamente transitiva o riflessiva.

Osserviamo immediatamente che il caso di grafo completo, e quindi di totale distinguibilità dei simboli, corrisponde all'entropia classica di una sorgente. Dimostreremo infatti che l'entropia di grafo generalizza l'entropia di Shannon.

Grazie ad alcune sue proprietà notevoli, in particolare una peculiare forma di subadditività, molti autori hanno impiegato l'entropia di grafo per dimostrare disuguaglianze di interesse combinatorico e algoritmico. 

Ad esempio lo stesso K\"orner ha ridimostrato una stima dovuta a Fredman e Komlos sul numero di funzioni di hash perfetto di un insieme.\footnote{Essenzialmente la stessa dimostrazione, tranne che K\"orner ha esplicitato il ricorso all'entropia di grafo.} K\"orner e Marton hanno poi migliorato tale stima sfruttando disuguaglianze più precise ricavate dall'analogo concetto di entropia di ipergrafo. 

Analizzando invece quando tale subadditività è in realtà una eguaglianza K\"orner et al. hanno dato una caratterizzazione dei grafi perfetti in termini di entropia, fornendo una nuova dimostrazione della congettura debole dei grafi perfetti. 

Kahn e Kim hanno esibito un algoritmo che determina adattivamente un ordine totale fissato ma ignoto. Ad ogni passo l'algoritmo fa ricorso al calcolo dell'entropia di un particolare grafo associato per determinare on-line il successivo confronto. Questo approccio consente anche di stimare il numero di estensioni lineari di un dato ordine parziale, la cui determinazione esatta è un problema nella classe \textsc{\#P-complete}. L'approccio di Kahn e Kim riconduce il calcolo dell'entropia a un problema di ottimizzazione convessa applicando il metodo dell'ellissoide. Cardinal et al. invece hanno esibito algoritmi polinomiali per lo stesso problema che non si avvalgono di tale metodo, ma sfruttano comunque l'entropia di grafo.

Nel seguito \(G = (V,E)\) denoterà un grafo semplice non diretto di insieme dei vertici \(V\) ed insieme degli archi \(E\). Ricordiamo che un insieme indipendente è un sottoinsieme di vertici non adiacenti e che una colorazione di \(G\) è una partizione di \(V\) in insiemi indipendenti. Chiameremo numero cromatico di \(G\) il minimo numero \(\chi(G)\) di insiemi indipendenti al variare delle colorazioni di \(G\). Infine assumeremo che tutti i logaritmi siano in base \(2\).

La rigorosa definizione di entropia di grafo comporta qualche difficoltà tecnica, la cui risoluzione costituisce l'obiettivo centrale di questo lavoro. Nella prossima sezione daremo tre possibili definizioni, mentre nella terza verrà dimostrata la loro equivalenza. Nell'ultima sezione verranno enunciate alcune proprietà di rilievo.
...
\subsection{Definizione in termini di VP(G)} 
\begin{definition}
	Sia \(G\) un grafo. Chiameremo \emph{politopo dei vertici di \(G\)} l'involucro convesso dei vettori caratteristici degli insiemi indipendenti di \(G\) e lo denoteremo \(\text{VP}(G)\). 
\end{definition}
\begin{example}
	...
	\end{pmatrix}
	\]
	ed il loro involucro convesso è \(\text{VP}(G)\).
...
\subsection{Equivalenza delle definizioni \eqref{eq:entropytwo} e \eqref{eq:entropythree}} 
\begin{theorem}
	...
	X\sim P}} I(X;Y)\ = \min_{\substack{\mathbf{a}\in \text{VP}(G) \\
	\mathbf{a}>0}} \sum_{i=1}^n p_i\log{\frac{1}{a_i}}\]
\end{theorem}
\begin{proof}
	...
	\[\min_{\substack{X\in Y\in S(G) \\
	X\sim P}} I(X;Y)\ \ge \min_{\substack{\mathbf{a}\in \text{VP}(G) \\
	\mathbf{a}>0}} \sum_{i=1}^n p_i\log{\frac{1}{a_i}}.\]
