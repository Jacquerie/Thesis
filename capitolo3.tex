...
\begin{algorithm}
	...
	\begin{algorithmic}
		[1] \STATE calcola \(x\in\text{STAB}(G)\) che realizzi il minimo dell'entropia con l'algoritmo di K\"orner e Marton
		    \IF{il contributo delle componenti rosse ad \(H(x)\) supera quelle delle componenti blu} scambia le catene \(A\) e \(B\). \ENDIF
		    \FOR{\(v\in A\cup B\)} \IF{\(v\) \`e un taglio} \ENDFOR
	\end{algorithmic}
\end{algorithm}
\begin{theorem}
