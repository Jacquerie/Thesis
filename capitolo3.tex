...
\begin{definition}
	Sia \(P\) un insieme parzialmente ordinato di sostegno \(V=\left\{v_1,\dots,v_n\right\}\), a propria volta dotato di un ordine totale \(\le\) fissato ma ignoto. Il \emph{problema dell'ordinamento con informazione parziale} consiste nel determinare l'ordine totale per mezzo di domande del tipo ``\(\text{\emph{è vero che}}\;v_i\le v_j\;\text{?}\,\)''. Chiameremo \emph{estensione lineare} un qualunque ordine totale che sia compatibile con \(P\) e denoteremo con \(e(P)\) il numero di estensioni lineari.
\end{definition}
Tale problema fu originariamente posto da Fredman nel 1976 \cite{Fredman1976}. Sempre Fredman dimostrò l'esistenza di un algoritmo che lo risolva compiendo \(\log{e(P)} + 2n\) confronti, il quale tuttavia richiede tempo di esecuzione superpolinomiale. Nel 1984 Kahn e Saks dimostrarono l'esistenza di un algoritmo che compia \(O(\log{e(P)})\) confronti \cite{Kahn1984}. Essi mostrarono infatti che esiste sempre una domanda della forma ``\(\text{\emph{è vero che}}\;v_i\le v_j\;\text{?}\,\)'' tale che le estensioni lineari per cui la risposta sia affermativa siano una parte compresa fra \(3/11\) e \(8/11\) del totale. Questo enunciato è un rilassamento della congettura \(1/3-2/3\), indipendentemente posta da Kislitsyn \cite{Kislitsyn1968}, Fredman e Linial \cite{Linial1984}.
\begin{theorem}
