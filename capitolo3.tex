...
\begin{theorem}[Kahn, Kim] \label{kktheorem} Sia \(P\) un insieme parzialmente ordinato di cardinalità \(n\). Allora vale
...
\end{lemma}
\begin{proof}
	È noto che l'entropia di un grafo su \(n\) vertici e dimensione massima di un insieme indipendente \(\alpha\) è maggiore o uguale a \(-\log{\frac{\alpha}{n}}\). La tesi segue applicando questo fatto a \(G=\overline{G}(P)\).\qed 
\end{proof}
\begin{theorem}
...
\end{theorem}
\begin{proof}
	Sia \(g(P)\) il numero di confronti necessario per ordinare \(P\). È chiaro che \[g(P)\le \log{n}\cdot(n-|C|),\]
	...
	e applicando il teorema \ref{cfjjmtheorem}
...
\end{proof}
\begin{theorem}
	\label{naivemergetheorem} Per ogni \(\varepsilon>0\) l'algoritmo \ref{naivemerge} compie al più \((1+\varepsilon)\log{e(P)}+(1+\varepsilon)(\log{e}+\log{(1+\frac{1}{\varepsilon})}+1)\cdot n\) confronti. 
\end{theorem}
\begin{proof}
  TODO\qed
\end{proof}
